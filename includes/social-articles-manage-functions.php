...
function get_category_list_options($post_id){
    global $socialArticles;
    $currentCategories = array();
    if(isset($post_id)){
        $currentCategories =  wp_get_post_categories($post_id);
    ...
    $categoryList = "<div class='categories-content'>";
    $categoryList .= '<select '.$select_type.' class="form-control" name="categories" id="'.$cat_id.'" onchange="closeCategoriesList(); return false;">';
    ...
    foreach($category_ids as $cat_id) {
        ...
        if(!in_array($cat_name, $categories_backList)){
            ...
            $categoryList .= '<option id="'.$cat_id.'" value="'.$cat_name.'" '.$selected.'/>'.$cat_name.'</option>';
        }
    }
    $categoryList .= "</select>";
    $categoryList .= "</div>";
...
function get_tags_list_options($post_id){            
    $currentTags = array();
    if(isset($post_id)){
       $currentTags =  wp_get_post_tags($post_id, array( 'fields' => 'ids' ));        
    }
    $select_type = "multiple";
    $tagsList = "<div class='tags-content'>";
    $tagsList .= '<select '.$select_type.' class="form-control" name="tags" onchange="closeTagsList(); return false;">';        
    ...
    foreach ( $tags as $key => $tag ) { 
      ...
      $tagsList .= '<option id="'.$tag_id.'" value="'.$tag_name.'" '.$selected.'/>'.$tag_name.'</option>';
